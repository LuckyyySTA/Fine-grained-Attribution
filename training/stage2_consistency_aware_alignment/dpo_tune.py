...
LONG_FORM_INSTRUCTION="Extract the relevant content from the provided documents and then use the extracted content to guide answer generation and cite the sources properly."
SHORT_FORM_INSTRUCTION="Extract the relevant content from the provided documents and then use the extracted content to provide a list of accurate answers for the given question. Always cite one and only one document for each answer. Separate answers by commas."
...
def encode_with_prompt_completion_format(example, tokenizer, max_seq_length):
    ...
    prompt_grounding = PROMPT_DICT["prompt_input"]

    doc_list = []
    for idx, doc in enumerate(example["docs"]):
        doc_text = f"Document [{idx+1}]: " + doc["text"]
        doc_list.append(doc_text)
    doc_prompt = "\n".join(doc_list)
    query_prompt = example["query"]

    if example["dataset"] == "short":
        instruction = SHORT_FORM_INSTRUCTION
    elif example["dataset"] == "long":
        instruction = LONG_FORM_INSTRUCTION
    example_input = f"Question: {query_prompt}\n\n{doc_prompt}"
    format_example = {"instruction": instruction, "input": example_input}
    query = prompt_grounding.format_map(format_example)

    def encode_target(query, target):
        examples_tokenized = _tokenize_fn(query + target, tokenizer, max_seq_length)
        sources_tokenized = _tokenize_fn(query, tokenizer, max_seq_length)
    ...
    if "grounding" in example.keys():
        if example["grounding"][-1] != ".":
            example["grounding"] = example["grounding"] + "."
        chosen_target = "[GROUNDING]" + example["grounding"] + "[ANSWER]" + example["chosen"]
        rejected_target = "[GROUNDING]" + example["grounding"] + "[ANSWER]" + example["rejected"]
    else:
        chosen_target = example["chosen"]
        rejected_target = example["rejected"]
    
    chosen_encoded = encode_target(query, chosen_target)
    rejected_encoded = encode_target(query, rejected_target)
...
def main():
    ...
    if "prompt" in raw_datasets["train_prefs"].column_names and "completion" in raw_datasets["train_prefs"].column_names:
        ...
        encode_function = partial(
            encode_with_prompt_completion_format, # change here
            tokenizer=tokenizer,
    ...
    if args.with_tracking:
        ...
        accelerator.init_trackers("stage2", experiment_config)
